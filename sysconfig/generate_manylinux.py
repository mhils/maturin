...
def main():
    well_known = defaultdict(lambda: defaultdict(list))
    cwd = os.getcwd()
    for arch in ARCHES:
        ...
        for ver in PY_VERS:
            ...
            for key in ["system", "platform", "base_prefix"]:
                metadata.pop(key, None)
            well_known["linux"][arch].append(metadata)

    with open("src/python_interpreter/sysconfig.json", "w") as f:
